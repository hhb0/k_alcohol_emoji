...
def generate_prompt(name, feature, situation_keyword, emotion_keyword):
    prompt = f"""
전통주 이름은 변경하지마세요.
고객이 원하는 키워드에 제품의 특징을 이유로 제품을 추천해 주세요.
...
예시)
'달홀진주25'는 얼굴에 진한 미소를 짓게 만드는 순수 증류주로 축제, 파티, 그리고 기념일 같은 즐거운 시간을 더욱 풍성하게 채워줍니다.
가족과 친구, 그리고 연인들과 함께하는 소중한 순간을 기념하고 축하하는데 딱 어울리며, 선물로도 인기가 높습니다.

옅은 핑크빛이 도는 스위트 와인으로, 장미향이 은은하게 나는 달콤한 디저트와인입니다. 
당도와 산도의 밸런스가 좋아 깔끔하고 단맛이 많은 만큼 식중주보다는 식전 주나 식후 주로 좋은 와인입니다. 
레드 다이아몬드의 색을 가진 와인으로 향이 바로 치고 오지는 않지만, 잔잔히 올라오는 포도향이 좋습니다. 
살짝 입을 대면 향에서 숨겨졌던 잘 익은 포도 맛이 그대로 올라옵니다. 
떫은맛, 타닌감, 산미는 지극히 적지만, 이 맛 하나로도 충분히 맛을 표현합니다.
